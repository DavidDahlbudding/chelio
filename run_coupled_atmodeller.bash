...
TOA_P=1e-1
BOA_P_THRESHOLD=1e6 # below this pressure, the atmosphere is probably too thin to
TEMP=200
ALBEDO=0.1
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --TEMP_MELT) TEMP_MELT="$2"; shift 2 ;;
        --MELT_FRAC) MELT_FRAC="$2"; shift 2 ;;
        --H_OCEAN) H_OCEAN="$2"; shift 2 ;;
        --CtoH) CtoH="$2"; shift 2 ;;
        --NtoC) NtoC="$2"; shift 2 ;;
        --fO2) fO2="$2"; shift 2 ;;
        --StoC) StoC="$2"; shift 2 ;;
        --CltoC) CltoC="$2"; shift 2 ;;
        --with_ggchem) with_ggchem="$2"; shift 2 ;;
...
mkdir -p "${CHELIO_PATH}/${OUT_DIR}/${NAME}_outgassed"
...
# if _tp.dat does not already exist, run this part
if [ ! -f "${CHELIO_PATH}/${OUT_DIR}/${NAME}_outgassed/${NAME}_outgassed_tp.dat" ]; then
    # Run python script to create initial abundance file for HELIOS
    python3 "${CHELIO_PATH}/source/calc_abundances_atmodeller.py" --output_dir "helios" \
        --melt_frac "$MELT_FRAC" --T_surf "$TEMP_MELT" \
        --H_ocean "$H_OCEAN" --CtoH "$CtoH" --NtoC "$NtoC" --fO2 "$fO2" # --StoC "$StoC" --CltoC "$CltoC"

    cp "${CHELIO_PATH}/helios_inputs/species.dat" "${CHELIO_PATH}/${OUT_DIR}/${NAME}_outgassed/species.dat"
    cp "${CHELIO_PATH}/helios_inputs/P_BOA.dat" "${CHELIO_PATH}/${OUT_DIR}/${NAME}_outgassed/P_BOA.dat"

    # read BOA_P from helios_inputs/P_BOA.dat
    ...
    echo "Running HELIOS with outgassed molecular species..."
    cd "${HELIOS_PATH}"
    python3 -u helios.py \
            -name "${NAME}_outgassed" \
...
python3 "${CHELIO_PATH}/source/calc_abundances_atmodeller.py" --output_dir "ggchem" \
    --melt_frac "$MELT_FRAC" --T_surf "$TEMP_MELT" \
    --H_ocean "$H_OCEAN" --CtoH "$CtoH" --NtoC "$NtoC" --fO2 "$fO2" # --StoC "$StoC" --CltoC "$CltoC"
...
cp "${CHELIO_PATH}/helios_inputs/species.dat" "${CHELIO_PATH}/${OUT_DIR}/${NAME}/species.dat"
cp "${CHELIO_PATH}/helios_inputs/P_BOA.dat" "${CHELIO_PATH}/${OUT_DIR}/${NAME}/P_BOA.dat"

BOA_P=$(cat "${CHELIO_PATH}/${OUT_DIR}/${NAME}/P_BOA.dat")
# TODO: adjust BOA_P after running GGchem (species could condense and mean molecular weight could change)
